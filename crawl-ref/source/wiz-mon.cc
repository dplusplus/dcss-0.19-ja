...
void wizard_create_spec_monster_name()
{
    ...
    {
        mprf(MSGCH_DIAGNOSTICS, jtrans("Such a monster couldn't be found."));
        return;
    }
    ...
    if (!in_bounds(place))
    {
        mprf(MSGCH_DIAGNOSTICS, jtrans("Found no space to place monster."));
        return;
    }
    ...
    {
        mprf(MSGCH_DIAGNOSTICS, jtrans("Unable to place monster."));
        return;
    }
...
void debug_list_monsters()
{
    ...
    for (int i = 0; i < MAX_MONSTERS; ++i)
    {
        ...
        if (prev_name != name && count > 0)
        {
            ...
            if (count > 1)
            {
                snprintf(buf, sizeof(buf), jtransc("%d {name}"), count,
                         prev_name.c_str());
            }
            else
    ...
    mpr_comma_separated_list(jtrans_notrim("Monsters: "), mons);
...
{
    mprf(MSGCH_PROMPT, jtrans_notrim("(c)hange spawn rate or (s)pawn monsters? "));
    ...
    if (c == 'c')
    {
        mprf(MSGCH_PROMPT, jtrans_notrimc("Set monster spawn rate to what? (now %d, lower value = higher rate) "),
        ...
        if (!cancellable_get_line(specs, sizeof(specs)))
        {
            ...
            if (rate || specs[0] == '0')
            {
                mprf(jtransc("Setting monster spawn rate to %i."), rate);
    ...
    else if (c == 's')
    {
        ...
        if (max_spawn <= 0)
        {
            mprf(MSGCH_PROMPT, jtrans("Level already filled with monsters, "
                                      "get rid of some of them first."));
            return;
        }

        mprf(MSGCH_PROMPT, jtrans_notrimc("Spawn how many random monsters (max %d)? "),
             max_spawn);
        ...
        if (!cancellable_get_line(specs, sizeof(specs)))
        {
            ...
            if (num > 0)
            {
                mprf(jtransc("Spawning %i monster%s."), num, num == 1 ? "" : "s");
...
void debug_stethoscope(int mon)
{
    ...
    {
        mprf(MSGCH_PROMPT, jtrans("Which monster?"));
    ...
    if (found_spell)
        mprf(MSGCH_DIAGNOSTICS, jtransc("spells: %s"), spl.str().c_str());
    ...
    if (found_item)
        mprf(MSGCH_DIAGNOSTICS, jtransc("inv: %s"), inv.str().c_str());
...
void wizard_detect_creatures()
{
    ...
    mprf(jtransc("Detected %i monster%s."), count, count == 1 ? "" : "s");
}
...
void wizard_dismiss_all_monsters(bool force_all)
{
    ...
    if (!force_all)
    {
        mprf(MSGCH_PROMPT, jtrans_notrim("What monsters to dismiss (ENTER for all, "
    ...
    mprf(jtransc("Dismissed %i monster%s."), count, count == 1 ? "" : "s");
...
{
    mprf(MSGCH_PROMPT, jtrans_notrim("Make the monster (S)hout or (T)alk? "));
...
{
    mprf(MSGCH_PROMPT, jtrans_notrim("Apply blessing of (B)eogh, The (S)hining One, or (R)andomly? "));
    ...
    if (!bless_follower(mon, god, true))
        mprf(jtransc("%s won't bless this monster for you!"),
             god_name_j(god).c_str());
}
...
void wizard_give_monster_item(monster* mon)
{
    ...
    if (!mon->take_item(player_slot, mon_slot))
        mpr(jtrans("Error: monster failed to take item."));
}
...
void wizard_make_monster_summoned(monster* mon)
{
    ...
    {
        mprf(MSGCH_PROMPT, jtrans("Monster is already summoned."));
        return;
    }
    ...
    mprf(MSGCH_PROMPT, jtrans("[a] clone [b] animated [c] chaos [d] miscast [e] zot"));
    mprf(MSGCH_PROMPT, jtrans("[f] wrath [h] aid   [m] misc    [s] spell"));

    mprf(MSGCH_PROMPT, jtrans_notrim("Which summon type? "));
    ...
    switch (choice)
    {
        ...
        case 's':
        {
            ...
            {
                mprf(MSGCH_PROMPT, jtrans("No such spell."));
                return;
            }
            type = (int) spell;
            break;
...
void wizard_polymorph_monster(monster* mon)
{
    ...
    {
        mprf(MSGCH_PROMPT, jtrans("Invalid monster type."));
        return;
    }
    ...
    if (!mon->alive())
    {
        mprf(MSGCH_ERROR, jtrans("Polymorph killed monster?"));
        return;
    }
...
void debug_pathfind(int idx)
{
    ...
    if (success)
    {
...
#endif
        string path_str;
        mpr(jtrans_notrim("Here's the shortest path: "));
        ...
        path_str = "";
        mpr("");
        mpr(jtrans_notrim("And here are the needed waypoints: "));
        for (coord_def pos : path)
            path_str += make_stringf("(%d, %d)  ", pos.x, pos.y);
        mpr(path_str);
...
void debug_miscast(int target_index)
{
    ...
    char specs[100];
    mprf(MSGCH_PROMPT, jtrans_notrim("Miscast which school or spell, by name? "));
    if (cancellable_get_line_autohist(specs, sizeof specs) || !*specs)
    {
        canned_msg(MSG_OK);
        return;
    ...
    if (spell != SPELL_NO_SPELL)
    {
        ...
        if (!disciplines)
        {
            mprf(jtransc("Spell '%s' has no disciplines."), spell_title_jc(spell));
            return;
        }
    }
    ...
    if (spell != SPELL_NO_SPELL)
        ...
        mprf(MSGCH_PROMPT, jtrans_notrim("Enter miscast_level or spell_power,raw_spell_failure: "));
...
void debug_ghosts()
{
    mprf(MSGCH_PROMPT, jtrans("(C)reate or (L)oad bones file?"));
